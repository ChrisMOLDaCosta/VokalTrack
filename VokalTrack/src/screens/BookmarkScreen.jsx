...
import { LatihanList } from '../data/latihan'; // contoh data statis
...
export default function BookmarkScreen() {
  ...
  const removeBookmark = (id) => {
    Alert.alert('Hapus Bookmark', 'Yakin ingin menghapus latihan ini?', [
        ...
        <View style={styles.listContainer}>
          {bookmarks.map(item => (
            <View key={item.id} style={styles.cardWrapper}>
              <ItemBookmark item={item} onPressRemove={removeBookmark} />
              <TouchableOpacity style={styles.deleteButton} onPress={() => removeBookmark(item.id)}>
